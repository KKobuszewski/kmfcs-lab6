...
*
* Brent's method makes use of the bisection method, the secant method, 
* and inverse quadratic interpolation in one algorithm.
*
* To Compile Please use flag -std=c++11.
*
********************************************************************************/
...
template<typename T>
void brents_fun(std::function<T (T)> f, T lower_bound, T upper_bound, T TOL, T MAX_ITER)
{
...
int main()
{
    ...
    for (function = 1; function <= 3; function++)
    {
        if (function == 1)
        {
            ...
            brents_fun<double>(f,a,b,TOL,MAX_ITER);
        }
        else if (function == 2)
        {
            a = -10;
