...
{   
    ...
    std::cout << "SIGN: " << sgn << std::endl;
        ...
        extract_wavefunction(wavefunctions->at(it),0.3,dx,sgn);
...
int main(int argc, char* argv[])
{
    ...
    std::ofstream file; file.open("potential_well.dat");
    ...
    unsigned it=0;
    for (double E = -200; E <= -40.0; E += dE)
    {
        it += shooting_method(&wavefunctions,&energies,E,E+dE,it,3*NX/4);
    }
    
    std::cout << std::endl;
    FILE* output_wf = fopen( "wavefunctions.bin", "w" );
